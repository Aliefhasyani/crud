        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b-2 border-gray-200 bg-gray-50">
                    ...
                    <th class="py-3 px-4 font-semibold text-sm text-gray-600">Company</th>
            ...
            <tbody>
                @foreach ($employers as $employer)
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td class="py-3 px-4 text-sm">{{ $employer->name }}</td>
                        <td class="py-3 px-4 text-sm text-gray-600">{{ $employer->company }}</td>
                        <td class="py-3 px-4 text-sm">
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('employer.delete', $employer->id) }}" class="inline m-0">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit" 
