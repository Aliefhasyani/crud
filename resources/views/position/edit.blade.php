...
<body class="bg-gray-50 text-gray-800 p-8">
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <h1 class="text-xl font-bold text-gray-700 mb-6">Edit Position</h1>

        <form method="POST" action="{{ route('position.update', $position->id) }}" class="space-y-5">
            @csrf
            @method('PUT') 
            ...
            <div class="flex flex-col gap-1.5">
                ...
                >
                    @foreach ($employers as $employer)
                        <option value="{{ $employer->id }}" {{ old('employer_id', $position->employer_id) == $employer->id ? 'selected' : '' }}>
                            {{ $employer->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="pt-2 flex items-center gap-3">
                ...
                <a href="{{ route('position.index') }}" class="text-sm text-gray-600 hover:text-gray-900 font-medium">
                    Cancel
                </a>
            </div>
        </form>
